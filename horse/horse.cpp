...
int main() {
    ...
    // --------------------
    GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "马", NULL, NULL);
    if (window == NULL) {
    ...
    // texture
    unsigned int texture = load_texture("grass-2.png");
    ...
    for (int i = 0; i < HORSE_NUMBER; i++) {
        horseModel.emplace_back(std::format("./horses/horses_{}.obj", i));
    }
    cout << "Load Success\n";
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // default diffuse for horse
        static glm::vec3 diffuse_color  = {0.631f, 0.384f, 0.27f};
        static glm::vec3 specular_color = {0.5f, 0.5f, 0.5f};
		...
		*/
        ...
        const static glm::vec3 init_position = {0.0f, 0.0f, 0.0f};
        static glm::vec3       translate     = {0.0f, 0.0f, 0.0f};
        static glm::vec3       rotate        = {0.0f, 1.0f, 0.0f};
        static float           angle         = glm::radians(51.0f);
        static float           scale         = 1.471f;
